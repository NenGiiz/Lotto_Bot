...
number_range = (0,999)  # ตัวเลขที่สามารถเลือกได้ 000-999
min_players = 1  # จำนวนผู้เล่นขั้นต่ำ
confirm_duration = 300  # ระยะเวลาในการยืนยันตัวตน (วินาที)
...
async def update_embed_time(announcement_message, embed):
    ...
    if remaining_time <= 0 and lotto_running:
        ...
        await announcement_message.channel.send("🕗 หมดเวลาการเข้าร่วมเกมแล้ว ต่อไปจะเป็นการประกาศรางวัลผู้โชคดี @everyone \n -------------------------")
        lotto_running = False
...
async def lotto(ctx, hour: int, minute: int):
    ...
    embed = discord.Embed(
        ...
        description=(f"@everyone กิจกรรมเริ่มขึ้นแล้ว! \n\nให้เลือกตัวเลขในรูปแบบ 3 หลัก เช่น 000-999 เพื่อเข้าร่วมเกม!\n"
                     "ตัวเลขที่เลือกจะต้องห้ามซ้ำกัน ใครพิมพ์ก่อนจะได้สิทธิ์เลขนั้นก่อน\n\nการได้สิทธิ์ในตัวเลขจะต้องมีเครื่องหมาย ✅ จากระบบ \nหลังจากเลือกตัวเลขแล้วเท่านั้น\n\n❗❗ การตัดสินของทีมงานถือเป็นที่สิ้นสุด ❗❗\n\n"
                     f"📌 **จะประกาศรางวัลในเวลา {hour:02d}:{minute:02d}** "
    ...
    await asyncio.sleep(5)
    await ctx.send("📣 และผู้โชคดีที่ชนะรางวัลได้แก่....")
    ...
    if confirmed_winners:
        ...
        await ctx.send(f"🎉 ผู้ชนะทั้งหมดที่ยืนยันตัวตนแล้ว: {winner_mentions} \n\nให้ทำการเปิด 🎟️ Ticket ในห้อง https://discord.com/channels/1326774367492898870/1330853563299266601 เพื่อรอรับรางวัลจากแอดมินได้เลย!. \n -------------------------")
    else:
